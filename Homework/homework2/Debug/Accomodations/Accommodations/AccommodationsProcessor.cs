    ...
    private static void ProcessCommand(string input)
    {
        ...
        switch (commandName)
        {
            case "book":
                ...
                // Добавил валидацию
                if (!Enum.TryParse<CurrencyDto>(parts[5], true, out CurrencyDto currency))
                {
                    Console.WriteLine($"Invalid currency: {parts[5]}");
                    return;
                }
            ...
            case "undo":
                // Обработка случая, когда нечего отменять
                if (s_commandIndex == 0)
                {
            ...
            case "search":
                ...
                // Добавил валидацию
                if (!DateTime.TryParse(parts[1], CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime startDate))
                {
                    Console.WriteLine("Invalid start date format");
                    return;
                }
                if (!DateTime.TryParse(parts[2], CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime endDate))
                {
